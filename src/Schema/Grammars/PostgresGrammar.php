...
class PostgresGrammar implements SchemaGrammar
{
    ...
    private function type(ColumnSchema $column): string
    {
        ...
        if ($column->autoIncrement) {
            return $column->type === ColumnSchema::BIG_INTEGER ? 'BIGSERIAL' : 'SERIAL';
        }

        return match ($column->type) {
            ColumnSchema::INTEGER => 'INTEGER',
            ColumnSchema::BIG_INTEGER => 'BIGINT',
            ColumnSchema::BOOLEAN => 'BOOLEAN',
            ColumnSchema::FLOAT => 'DOUBLE PRECISION',
            ColumnSchema::DATETIME => 'TIMESTAMP',
